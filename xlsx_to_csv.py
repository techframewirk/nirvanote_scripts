...
if Path('xlsx-info.json').exists():
    ...
    for district in districts:
        ...
        for date in dates:
            ...
            intra_files = [str(Path(date)/file) for file in os.listdir(date) if Path(Path(date)/file).is_file() and file != 'inter.xlsx' and file != '.DS_Store' and not file.startswith(".") and file.endswith(".xlsx")]
